...
import { CheckCircle, Zap, X, Wallet, AlertTriangle, Hash, UserCheck } from "lucide-react";
...
type ModalState =
  | { stage: "form" }
  | { stage: "success"; traderName: string };
...
export default function MembershipPlansClient({ plans, activeMembershipPlanId, walletBalance }: Props) {
  ...
  const [modal, setModal] = useState<ModalState>({ stage: "form" });
  ...
  function confirm() {
    ...
    startTransition(async () => {
      ...
      if (!result.success) {
        ...
        setModal({ stage: "success", traderName: result.traderName });
        setTimeout(() => {
          setSelected(null);
          router.refresh();
        }, 4000);
          ...
          return (
            <div key={plan.id} className={`bg-[#111] border ${accent.border} rounded-xl p-6 flex flex-col`}>
              ...
              <div className="mb-5">
                ...
                {plan.duration && (
                  <span className="text-[13px] text-[#555] ml-1">/ {Math.round(plan.duration / 7)} weeks</span>
                )}
              </div>
          ...
          <div className={`relative w-full max-w-md bg-[#111] border ${(PLAN_ACCENTS[selected.name] ?? DEFAULT_ACCENT).modal} rounded-2xl shadow-2xl`}>
            ...
            {modal.stage === "success" ? (
              <div className="px-6 py-10 flex flex-col items-center text-center">
                ...
                <h3 className="text-[18px] font-bold text-white mb-2">Subscription Activated!</h3>
                ...
                <div className="w-full bg-[#1a1a1a] border border-[#2a2a2a] rounded-xl p-4 mb-5">
                  ...
                  <p className="text-[20px] font-bold text-[#f0b429]">{modal.traderName}</p>
                  <p className="text-[12px] text-[#555] mt-2 leading-relaxed">
                    Please reach out to <span className="text-white font-medium">{modal.traderName}</span> to get started with your mentorship plan. They will guide you through the next steps.
                  </p>
                </div>
              ...
              <>
                ...
                <div className="flex items-center justify-between px-6 py-5 border-b border-[#1e1e1e]">
                  <div>
                    ...
                    <p className="text-[12px] text-[#555] mt-0.5">Enter your trading code to activate</p>
                  </div>
                ...
                <div className="px-6 py-5 space-y-5">
                  ...
                  {!canAfford && (
                    <div className="flex items-start gap-2.5 p-3 bg-[#ef4444]/10 border border-[#ef4444]/20 rounded-lg">
                      ...
                      <p className="text-[12px] text-[#ef4444]">Insufficient balance. Please deposit funds before subscribing.</p>
                    </div>
                  )}
                ...
                <div className="flex gap-3 px-6 pb-6">
                  ...
                  <button type="button" onClick={confirm} disabled={pending || !canAfford} className="flex-1 py-2.5 text-[13px] font-bold rounded-lg bg-[#f0b429] hover:bg-[#e0a424] text-black transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
